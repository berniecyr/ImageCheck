...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    # Graceful shutdown — catches both Ctrl-C and SIGTERM (service managers).
    # The flag lets loops drain cleanly rather than being interrupted mid-file.
    # ------------------------------------------------------------------
    ...
    if cfg["rtsp_enabled"] and cfg["rtsp_location"]:
        ...
        logger.info("[RTSP] Live monitoring disabled (rtsp_enabled=false or missing IP).")

    # --- Sighthound background processor ----------------------------
    logger.info("[Sighthound] Starting background tasks...")
            ...
            try:
                if path.lower().endswith((".mp4", ".avi", ".mov", ".mkv")):
                    ...
                    if img is not None:
                        ...
                        logger.warning(f"[{idx}/{total}] Could not read image: {display}")
                        error_count += 1
                        continue
                ...
                logger.error(f"[{idx}/{total}] Error processing {display}: {exc}")
                error_count += 1
    ...
    while not _shutdown.is_set():
        ...
        try:
            ...
            logger.error(f"Cannot list inbox: {e}")
        ...
        for item in media_files:
            ...
            try:
                ...
                if nudity_found or event_fired:
                    ...
                    logger.info(f" -> ALERT ({reason}). Moved to Retained_Media.")
                else:
                    os.remove(path)
                    logger.info(f" -> Clean: no detections. Deleted {clean_name}")

            except Exception as e:
                logger.error(f" -> ERROR routing {clean_name}: {e}")

        logger.info("[AI] Batch complete. Returning to watch mode...")
        logger.info(f" -> Processed loop in {time.time() - script_start:.2f}s")

    logger.info("ImageCheck shut down cleanly.")
